...
namespace TcpComm
{
   ...
   public sealed class RingBuffer : Disposable {
      ...
      /// </summary>
       //private const int DEFAULT_SIZE = 4098;
      ...
      public int IndexOf(byte[] value, int valueCount, int index = 0)
      {
          int i = index;
          while (i < this.Count)
          {
              int j = 0;
              while (j < valueCount && i<this.Count )
              {
                  if (this[i] == value[j])
                  {
                      j++;
