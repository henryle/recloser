...
{
    public partial class HistoryTuBufrm : Form
    {
        private string _devicefile  = string.Empty;
        
        public HistoryTuBufrm(string DeviceFile)
        {
            InitializeComponent();
            _devicefile = DeviceFile;
        }
        List<TuBu> _list;
        public HistoryTuBufrm(List<TuBu> list)
        {
            InitializeComponent();
            _list = list;
        }
        ...
        private void HistoryTuBufrm_Load(object sender, EventArgs e)
        {
            
            //cboDevice.DataSource = DeviceStatic.GetDevices(_devicefile).OfType<TuBu>().ToList<TuBu>();//Program.GetDevices().OfType<TuBu>();
        ...
        public void ExportGridToExcel(string ExportingFileName, DataGridView grdSearchResult)
        {
            ...
            string strFields = "DeviceId \t Name \t Location \t Opt  \t OperationTime  \t Mod";
            string strvalues;
            ...
            {
                strvalues = row["DeviceId"].ToString() + " \t " + row["Name"].ToString() + " \t " + row["Location"].ToString() + " \t " + row["Opt"].ToString() + " \t " + row["OperationTime"].ToString() + "\t" + row["Mod"].ToString();
        ...
        private void btnFind_Click_1(object sender, EventArgs e)
        {
            ...
            grdResults.DataSource = DBController.Instance.SearchTuBu(id, from, to);
        }
    }
}
