...
namespace RecloserAcq
{
    public partial class formOperationLog: Form
    {
        ...
        public formOperationLog(string DeviceFile)
        {
            InitializeComponent();
            _devicefile = DeviceFile;
        }
        List<RecloserBase> _list;
        public formOperationLog(List<RecloserBase> list)
        {
            InitializeComponent();
            _list = list;
        }
        ...
        public void ExportGridToExcel(string ExportingFileName, DataGridView grdSearchResult)
        {
            ...
            string strFields = "DeviceId \t Name \t Location \t Opt  \t OperationTime  \t Mod \t UserId \t Username ";
            string strvalues;
        ...
        {
            int id = -1;
               id = Convert.ToInt32(cboDevice.SelectedValue);
            ...
            grdResults.DataSource = DBController.Instance.searchOperationLog(from, to, id);
        }

        private void btnFindAllDev_Click(object sender, EventArgs e)
        {
            ...
            grdResults.DataSource = DBController.Instance.searchOperationLog(from, to);
        }
    }
}
