...
{    
    public class NulecVP: RecloserBase
    {
        ...
        { 
            ...
            while(i < data.Length - 1)
            {
                ...
                if (data[i] == Parser.NulecVP_StartBlock[0] && data[i + 1] == Parser.NulecVP_StartBlock[1])
                {
                    int j = i + 2;
                    while (j < data.Length - 2)
                    {
                        if (data[j] == Parser.NulecVP_EndBlock[0] &&
                            data[j + 1] == Parser.NulecVP_EndBlock[1] &&
                            data[j + 2] == Parser.NulecVP_EndBlock[2])
                        {
        ...
        private void GetValue(string s)
        {
            ...
            {
                LogService.Logger.Error(string.Format("NulecVP.GetValue({0}) error", s), ex);
            }
        }
        ...
        public override void WriteRow()
        {
            ...
            try
            {
                ...
                {
                    LogService.WriteError("WriteRow NulecVP 1", e.Message);
                }
            }
            catch (Exception ex)
            {
                LogService.WriteError("WriteRow NulecVP", ex.Message);
            }
        }
    }
}
