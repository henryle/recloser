...
namespace RecloserAcq.Device
{
    public class CooperFXB: RecloserBase
    {
        ...
        {
            _buffersize = 128;
            this.StartPackage = "00 05 00 00 01 FA";
        }
        ...
        public override void UpdateData(byte[] data)
        {
            try
            {
                switch (data.Length - 1)
                {
                    ...
                    case DataLength_Status_2:
                        if (currentRequest.Name == "OpenClose")
                            UpdateStatus_2(data);
                        else
                            UpdateBattery(data);
                        break;
            ...
            {
                FA_Accounting.Common.LogService.Logger.Error(string.Format("Cooper UpdateData error"), ex);
            }
        ...
        private void UpdateStatus_2(byte[] data)
        {
            if (data.Length - 1 != DataLength_Status_2)
                FA_Accounting.Common.LogService.WriteError("Cooper UpdateStatus 2", "Length of status 1 data is not valid");
            ...
            this.Status_Close = (rStatus & 1 << 4) != 0;
        ...
        private void UpdateStatus_1(byte[] data)
        {
            if (data.Length - 1 != DataLength_Status_1)
                FA_Accounting.Common.LogService.WriteError("Update Status 1", "Length of status 2 data is not valid");
        ...
        private void UpdateBattery(byte[] data)
        {
            if (data.Length - 1 != DataLength_Battery)
                FA_Accounting.Common.LogService.WriteError("Update Battery 1", "Length of battery data is not valid");
        ...
        private void UpdateCurrent(byte[] data)
        {
            if (data.Length - 1 != DataLength_Current)
                FA_Accounting.Common.LogService.WriteError("Update curency", "Length of current data is not valid");
...
//#endif
        ...
        public override void CommandOpen(bool auto)
        {
            ...
            //Listener.Send(TcpComm.Ultility.FromHex(Openstr));
            //System.Threading.Thread.Sleep(10);
            base.CommandOpen();
        }
        public override void CommandReclose()
        {
            ...
            //Listener.Send(TcpComm.Ultility.FromHex(ReClosestr));
            //System.Threading.Thread.Sleep(10);
            base.CommandReclose();
        }
        public override void SetTime()
        {
            Listener.Send(TcpComm.Ultility.FromHex(Settimestr));
            //System.Threading.Thread.Sleep(10);
        }
        public override void WriteRow()
        {
            ...
            try
            {
                ...
                {
                    FA_Accounting.Common.LogService.WriteError("Write Row Cooper", e.Message);
                }
            }
            catch (Exception ex)
            {
                FA_Accounting.Common.LogService.WriteError("Write Row Cooper", ex.Message);
            }
        }
        protected override void CheckingInsAlert()
        {
            //Listener.Send(Ultility.FromHex());
        }
