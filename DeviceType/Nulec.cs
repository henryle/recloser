...
{    
    public class Nulec: RecloserBase
    {
        ...
        public override eAlertOpenClose CheckCosphi()
        {
            ...
            try
            {
                ...
                else if (_powerFactor < this.MinOpen)
                {
                    secondcountmaxclose = 0;
                    tmpDateTimeMax = DateTime.MinValue;
        ...
        public override void WriteRow()
        {
            ...
            try
            {
                ...
                strFields = "DeviceId,Type,Date,Alert,Mo,Dong,DeviceTime,Operations,Amp12,Amp34,Amp56,AmpEarth" +
                    ",ApparentPower,ReactivePower,RealPower,PowerFactor,Battery,Battery_Voltage \r\n";
                ...
                {
                    FA_Accounting.Common.LogService.WriteError("WriteRow Nulec 1", e.Message);
                }
            }
            catch (Exception ex)
            {
                FA_Accounting.Common.LogService.WriteError("WriteRow Nulec", ex.Message);
            }
        }
        protected override void SendHandshake()
        {
            string request = "1B 32 01 2A 0A 52 45 51 0A 4F 53 0A 7E 0A 34 42 46 37 03";
            Listener.Send(Ultility.FromHex(request));
            FA_Accounting.Common.LogService.WriteInfo("Send StartComm", "");
            /*request = "";
            Listener.Send(Ultility.FromHex(request));*/

        }
        protected override void CheckingInsAlert()
        {
            //check if handshake is required
            FA_Accounting.Common.LogService.WriteInfo("CheckingInsAlert", "Start");
            try
            {
                
                string oprequest = "1B 32 01 2A 0A 52 45 51 0A 4F 50 0A 7E 0A 2A 0A 52 45 51 0A 55 57 23 0A 7E 0A 2A 0A 52 45 51 0A 56 57 23 0A 7E 0A 2A 0A 52 45 51 0A 57 57 23 0A 7E 0A 31 39 43 32 03";
                Listener.Send(Ultility.FromHex(oprequest));
            }
            catch (Exception ex)
            {
                FA_Accounting.Common.LogService.WriteError("Sending error", ex.Message);
            }
        }
        private readonly byte Nulec_Answer_Start = 0x01;
        private readonly byte Nulec_End = 0x03;
