    ...
    public class Elster1700: RecloserBase
    {
        ...
        public override void UpdateData(byte[] data)
        {
           ...
           // replyList.Add(parse);
            ...
            {
                FA_Accounting.Common.LogService.WriteError("updatedata", ex.ToString());
            }
        }
        #region old function 
        /*
        ...
        */
        ...
        public override void WriteRow()
        {
            ...
            {
                FA_Accounting.Common.LogService.WriteError("WriteRow Elster",ex.Message);
            }
        }
        protected override void StartSendingRequests()
        {
            timerBetweenEachPoll.Interval = this.Latencybetweenpoll;
            if (timerBetweenEachPoll.Interval < 1000 * 60 * 10)
            {
                timerBetweenEachPoll.Interval = 1000 * 60 * 10; //10 phut
            }
            ...
            this.pollingcount++;
            if (pollingcount % 2 == 0 && !this.CheckConnectionStatus())
            {
                return;
            }
            ...
            {
                FA_Accounting.Common.LogService.WriteError("StartSending Request", ex.Message);
            }
            finally
            {
        ...
        public void sendConnectCommand()
        {
            ...
            //}
            sleep(1); // in other code: sleep 1/4 second
            SendBaudRate();
            //Wait();
        ...
        private void SendBaudRate()
        {
            ...
            switch (BaudRate)
            {
                ...
                case 19200:/*Z=6*/
                    ...
                    break;
                default:
                    throw new Exception(BaudRate + "Invalid baudrate! Not support" );
        ...
        protected override void SaveData()
        {
                ...
                {
                    DBController.Instance.SaveElster(this);
                }
                
            
        }
       
        //private readonly byte Elster1700_Answer_NAK = 0x15; // =(int)21 NAK return 1 byte eg. when access denied
        private readonly byte Elster1700_Answer_ACK = 0x06; //ACK
        //private readonly byte Elster1700_Answer_Endx03 = 0x03;
        //private readonly byte Elster1700_Answer_End_close_quotation = 0x29; //)
        private readonly byte[] Elster1700_Answer_End_closeX03 = new byte[] { 0x29, 0x03 }; //)
        private readonly byte[] Elster1700_Answer_End_0X03 = new byte[] { 0x30, 0x03 }; //)
        //Completion character (CR, carriage return, code 0DH; LF, line feed, code 0AH).
        private readonly byte[] Elster1700_Answer_EndLineEnd = new byte[] { 0x0D, 0x0A };
        private readonly byte Elster1700_Answer_NAK = 0x15; // =(int)21 NAK return 1 byte eg. when access denied
