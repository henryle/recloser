...
{    
    // Recloser Vinh Phuc su dung con duoc goi la Recloser ADVC
    public class RecloserADVC: RecloserBase
    {
        ...
        //protected List<String> dataReceiveList = new List<string>();
        ...
        public virtual string Pattern_Current
        {
            get
            {
        ...
        public virtual string Pattern_Power
        {
            get
            {
                return @"(?<Type>KW#|KW[ABC]#|PWFT#|PWFT[ABC]#|MK#|MK[ABC]#|MR#|MR[ABC]#)\n(?<Value>-?\d{2,7}),(?<Factor>\d{3,4})";
            }
            set { }
        }
        public virtual string Pattern_Voltage
        {
            get
            {
                return @"(?<Type>MAV#|MBV#|MCV#|MEV#|MFV#|MDV#|MGV#|MHV#|MIV#)\n(?<Value>-?\d{1,7}),(?<Factor>\d{1,4})";
            }
            set { }
        }
        ...
        public override eDeviceType DeviceType
        {
            get
            {
                return eDeviceType.RecloserADVC;
            }
        }
        
        protected List<string> rawDataList = null;
        ...
        protected override void SaveData()
        {
                ...
                DBController.Instance.SaveRecloserADVC(this);
        ...
        public const string Pattern_Operation = @"OP\n(?<Operation>\d+)";
        protected bool getOperation(string s)
        {// Operation
        ...
        protected void GetValue(string s)
        {
            try
            {
            ...
            {
                FA_Accounting.Common.LogService.Logger.Error(string.Format("RecloserADVC.GetValue({0}) error", s), ex);
            }
        }
        protected bool? hastoCalculateVoltage = null;
        protected void GetMeasure()
        {
            ...
            VA_B_load = Math.Round(Vab); 
            VB_C_load = Math.Round(Vbc);
            VC_A_load = Math.Round(Vca);
            VA_GND_load = volt_phase_G[0] * 1000;
            VB_GND_load = volt_phase_G[1] * 1000;
            VC_GND_load = volt_phase_G[2] * 1000;
            VA_B_src = Math.Round(Vab);
            VB_C_src =  Math.Round(Vbc);
            VC_A_src = Math.Round(Vca);
            VA_GND_src = volt_phase_G[0] * 1000;
            VB_GND_src = volt_phase_G[1] * 1000;
            VC_GND_src = volt_phase_G[2] * 1000;
        ...
        protected bool GetVoltage1(string s)
        {
            ...
            if (pos_cmd >= 0)
            {
                ...
                {
                    FA_Accounting.Common.LogService.WriteError("Get Recloser VP Voltage", ex.Message);
                    return false;
                }
        ...
        protected virtual bool GetVoltage(string s)
        {
            ...
            if (match.Success)
            {
                ...
                switch (type)
                {
                    case "MAV#":
                        ...
                        this.VA_GND_load = c; //set it = 0 to test alert or set it > maxamp to test alert
        ...
        protected bool GetStatus(string s)
        {
        ...
        protected bool GetOtherValue(string s)
        {
            // Battery
        ...
        //protected bool GetDateTime(string s)
        //{
        //    // DateTime
        //    try
        //    {
        ...
        #region new field for RecloserADVC
        protected DateTime? _deviceTime;
        ...
        public double Current_IA
        {
            ...
            set
            {
                ...
                else
                {
                    ...
                    if (AmpPercent != 0 && _current_IB!=0 && _current_IC!=0 && 
                        ((Math.Max(Math.Max(value, _current_IB), _current_IC) 
                        - Math.Min(Math.Min(value, _current_IB), _current_IC)) * 3 / (value + _current_IB + _current_IC) >= p))
                    {
                        //if (_alertVal == 0)
                        //{
                            FA_Accounting.Common.LogService.WriteInfo("Bao dong pha", "");
                            _alertVal = 1;
                            SaveData();
                            Notify("AlertVal");
            ...
            set
            {
                ...
                else
                {
                    ...
                    if (AmpPercent != 0 && _current_IA!=0 && _current_IC!=0 &&  ((Math.Max(Math.Max(value,_current_IA),_current_IC)-Math.Min(Math.Min(value,_current_IA),_current_IC))*3/(value + _current_IA +  _current_IC)>= p))
                    {
                        //if (_alertVal == 0)
                        //{
                        FA_Accounting.Common.LogService.WriteInfo("Bao dong pha B", "");
                            _alertVal = 1;
                            SaveData();
                            Notify("AlertVal");
            ...
            set
            {
                ...
                else
                {
                    ...
                    if (AmpPercent != 0 && _current_IB != 0 && _current_IA != 0 && ((Math.Max(Math.Max(value, _current_IA), _current_IB) - Math.Min(Math.Min(value, _current_IA), _current_IB)) * 3 / (value + _current_IA + _current_IB) >= p))
                    {
                        //if (_alertVal == 0)
                        //{
                        FA_Accounting.Common.LogService.WriteInfo("Bao dong pha C", "");
                            _alertVal = 1;
                            SaveData();
                            Notify("AlertVal");
        ...
        protected double _apparent_S_A;
        [XmlIgnore]
        ...
        protected double _apparent_S_B;
        [XmlIgnore]
        ...
        protected double _apparent_S_C;
        [XmlIgnore]
        ...
        protected double _reactive_Q_A;
        [XmlIgnore]
        ...
        protected double _reactive_Q_B;
        [XmlIgnore]
        ...
        protected double _reactive_Q_C;
        [XmlIgnore]
        ...
        public double VC_A_load
        {
            ...
            set
            {
                if (_vC_A_load != value)
                {
                    _vC_A_load = value;
                   /*Bao dong theo Ap cu
                    * if (value != 0 && _volStandard != 0 && _volpercent != 0 &&
                        (value < (_volStandard - (_volStandard * _volpercent / 100)) || value > (_volStandard + (_volStandard * _volpercent / 100))))
                    {
                        _alertVal = 1;
                        SaveData();
                        Notify("AlertVal");
        ...
        #endregion new fields for RecloserADVC
        ...
        protected DateTime tmpDateTimeMax = DateTime.MinValue;
        protected DateTime tmpDateTimeMin = DateTime.MinValue;
        protected int secondcountmaxclose =0;
        protected int secondcountminopen = 0;
        ...
        public override void CommandClose(bool auto)
        {
           //VP xong
            ...
            try
            {
                //                    1B 32 01 2A 0A 53 45 54 0A 43 42 0A 31 0A 7E 0A 44 43 36 41 03 1B 32 01 2A 0A 52 45 51 0A 43 42 0A 7E 0A 41 44 43 46 03
        ...
        public override void WriteRow()
        {
            ...
            string filePath = _devicepath + Id + ".csv";
            string strFields = "";
            string strvalues = "";
            try
            {
	...
	"Current_IB,Current_IC,Current_IE,ApparentPower,Apparent_S_A,Apparent_S_B,Apparent_S_C,RealPower,Real_P_A," +
	"Real_P_B,Real_P_C,ReactivePower,Reactive_Q_A,Reactive_Q_B,Reactive_Q_C,Cosphi,Cosphi_A,Cosphi_B,Cosphi_C," +
    ...
    "VB_GND_load,VC_GND_load,VA_B_src,VB_C_src,VC_A_src,VA_B_load,VB_C_load,VC_A_load,Operation \r\n";
                ...
                {
                    FA_Accounting.Common.LogService.WriteError("WriteRow RecloserADVC 1", e.Message); 
                }
            }
            catch (Exception ex)
            {
                FA_Accounting.Common.LogService.WriteError("WriteRow RecloserADVC", ex.Message);
            }
        }
        protected override void StartSendingRequests()
        {
            ...
            {
                FA_Accounting.Common.LogService.WriteError("RecloserADVC_StartSendingRequest", ex.ToString());
        ...
        public virtual bool sendConnectCommand()
        {
            try
            {
                timerBetweenEachPoll.Stop();
            ...
            {
                FA_Accounting.Common.LogService.WriteError("RecloserADVC_sendConnectCommand", ex.ToString());
                return false;
            }
        ...
        protected bool GetTotalPower(string s)
        {
        ...
        protected virtual bool GetCurrent(string s)
        {
        ...
        protected virtual bool GetPower(string s)
        {
        ...
        public override void SendAmpRequest()
        {
            try
            {
                //2*REQMA#~*REQMB#~*REQMC#~*REQME#~14A0
                //1B 32 01 2A 0A 52 45 51 0A 4D 41 23 0A 7E 0A 2A 0A 52 45 51 0A 4D 42 23 0A 7E 0A 2A 0A 52 45 51 0A 4D 43 23 0A 7E 0A 2A 0A 52 45 51 0A 4D 45 23 0A 7E 0A 31 34 41 30 03
                Listener.Send(new byte[] { 0x1B, 0x32, 0x01, 0x2A, 0x0A, 0x52, 0x45, 0x51, 0x0A, 0x4D, 0x41, 0x23, 0x0A, 0x7E, 0x0A, 0x2A, 0x0A, 0x52, 0x45, 0x51, 0x0A, 0x4D, 0x42, 0x23, 0x0A, 0x7E, 0x0A, 0x2A, 0x0A, 0x52, 0x45, 0x51, 0x0A, 0x4D, 0x43, 0x23, 0x0A, 0x7E, 0x0A, 0x2A, 0x0A, 0x52, 0x45, 0x51, 0x0A, 0x4D, 0x45, 0x23, 0x0A, 0x7E, 0x0A, 0x31, 0x34, 0x41, 0x30, 0x03 });
        ...
        private readonly byte Answer_End = 0x03;
        public override byte[] DoParse(RingBuffer ringBuffer)
        {
            ...
            {
                int end = ringBuffer.IndexOf(Answer_End, start + 1);
